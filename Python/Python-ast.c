...
static PyTypeObject *AssExpr_type;
static char *AssExpr_fields[]={
    "target",
    "value",
};
...
static int init_types(void)
{
    ...
    AssExpr_type = make_type("AssExpr", expr_type, AssExpr_fields, 2);
    if (!AssExpr_type) return 0;
...
expr_ty
AssExpr(expr_ty target, expr_ty value, int lineno, int col_offset, PyArena
        *arena)
{
    expr_ty p;
    if (!target) {
        PyErr_SetString(PyExc_ValueError,
                        "field target is required for AssExpr");
        return NULL;
    }
    if (!value) {
        PyErr_SetString(PyExc_ValueError,
                        "field value is required for AssExpr");
        return NULL;
    }
    p = (expr_ty)PyArena_Malloc(arena, sizeof(*p));
    if (!p)
        return NULL;
    p->kind = AssExpr_kind;
    p->v.AssExpr.target = target;
    p->v.AssExpr.value = value;
    p->lineno = lineno;
    p->col_offset = col_offset;
    return p;
}
...
ast2obj_expr(void* _o)
{
    ...
    case BoolOp_kind:
        ...
        break;
    case AssExpr_kind:
        result = PyType_GenericNew(AssExpr_type, NULL, NULL);
        if (!result) goto failed;
        value = ast2obj_expr(o->v.AssExpr.target);
        if (!value) goto failed;
        if (_PyObject_SetAttrId(result, &PyId_target, value) == -1)
            goto failed;
        Py_DECREF(value);
        value = ast2obj_expr(o->v.AssExpr.value);
        if (!value) goto failed;
        if (_PyObject_SetAttrId(result, &PyId_value, value) == -1)
            goto failed;
        Py_DECREF(value);
...
obj2ast_expr(PyObject* obj, expr_ty* out, PyArena* arena)
{
    ...
    isinstance = PyObject_IsInstance(obj, (PyObject*)AssExpr_type);
    if (isinstance == -1) {
        return 1;
    }
    if (isinstance) {
        ...
        if (tmp == NULL) {
            PyErr_SetString(PyExc_TypeError, "required field \"target\" missing from AssExpr");
            return 1;
        }
        else {
            int res;
        ...
        if (tmp == NULL) {
            PyErr_SetString(PyExc_TypeError, "required field \"value\" missing from AssExpr");
            return 1;
        }
        else {
            int res;
        ...
        *out = AssExpr(target, value, lineno, col_offset, arena);
        if (*out == NULL) goto failed;
        return 0;
    }
...
PyInit__ast(void)
{
    ...
    if (PyDict_SetItemString(d, "BoolOp", (PyObject*)BoolOp_type) < 0) return
        NULL;
    if (PyDict_SetItemString(d, "AssExpr", (PyObject*)AssExpr_type) < 0) return
        NULL;
